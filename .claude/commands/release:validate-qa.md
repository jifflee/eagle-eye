...
### 7. Public-Readiness Check (Public Repos Only)

**For external (public) repos, run additional public-readiness validation:**
...
if [ -f "config/repo-profile.yaml" ]; then
  ...
  if [[ "$VISIBILITY" == "public" ]]; then
    echo "Running public-readiness check (external repository)..."

    # Run external deployment check
    ./scripts/ci/validators/external-deployment-check.sh
    READINESS_EXIT=$?

    if [ $READINESS_EXIT -eq 1 ]; then
      GATE_PUBLIC_READINESS="❌ FAIL - Public-readiness check failed"
      EXIT_CODE=1
    else
      GATE_PUBLIC_READINESS="✅ PASS - Ready for external deployment"
    fi
  else
    GATE_PUBLIC_READINESS="⚠️ SKIP - Not a public repository"
  fi
else
  GATE_PUBLIC_READINESS="⚠️ SKIP - No repo profile found"
fi
```

**Public-readiness check includes:**
- Sensitive data leakage detection
- Documentation completeness (README, LICENSE)
- Dependency safety validation
- Code safety checks

**Update the gates table to include:**

```bash
echo "| Public Readiness | $GATE_PUBLIC_READINESS |"
```
...
- **NEW:** Includes public-readiness validation for external repos
- **NEW:** Validates repository is safe for public release before QA promotion
- **NEW:** Uses external-deployment-check.sh for comprehensive validation
- See issue #176 for requirements
- Parent epic: #172
- Related: #1129 (unified SDLC for public/private repos)
