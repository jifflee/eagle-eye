...
argument-hint: "[--dry-run] [--changelog] [--auto-merge] [--milestone X] [--selective] [--list-candidates] [--no-auto-promote] [--no-auto-fix] [--wait]"
---
...
### 7. Automated Release Pipeline (post-merge)

After the qa PR is merged, automatically trigger the release pipeline unless `--no-auto-promote` is set:

```bash
# Trigger automated pipeline: validate-qa → [auto-fix] → promote-main
./scripts/release-pipeline.sh \
  --qa-pr "$PR_NUMBER" \
  ${NO_AUTO_FIX:+--no-auto-fix} \
  ${NO_AUTO_PROMOTE:+--no-auto-promote} \
  ${WAIT:+--wait}
```

**Pipeline flow:**
```
release:promote-qa (PR merged to qa)
  → Auto-trigger: release:validate-qa
  → If PASS:  Auto-trigger: release:promote-main → done
  → If FAIL:
      → Auto-create issue for each failing gate (linked to QA PR)
      → Launch containers to fix each issue
      → Re-run release:validate-qa (max 2 retry cycles)
      → If PASS: Auto-trigger: release:promote-main
      → If FAIL after retries: Escalate to user
```
...
| Flag | Behavior |
|------|----------|
| `--no-auto-promote` | Run validate-qa but skip release:promote-main |
| `--no-auto-fix` | Report gate failures only; skip issue creation and containers |
| `--wait` | Block until full pipeline completes |

**Exit codes from pipeline:**
- `0`: Pipeline complete — promote-main triggered (or skipped by `--no-auto-promote`)
- `1`: Pipeline blocked — gate failures after max retries (or `--no-auto-fix`)
- `2`: Passed with warnings — promote-main triggered, warnings passed through
- `3`: Pipeline error — unexpected failure

**Note:** Warnings (non-blocking gates) are passed through to the promote-main PR body.
If `--no-auto-promote` is set, invoke `/release:promote-main` manually after validating.
...
- **After merge:** Automated pipeline runs validate-qa → promote-main (Issue #1267)
  - Use `--no-auto-promote` to disable auto-promotion to main
  - Use `--no-auto-fix` to disable auto-fix containers (report only)
  - Use `--wait` to block until full pipeline completes
  - Max 2 auto-fix retry cycles before escalation to user
...
- [scripts/release-pipeline.sh](/scripts/release-pipeline.sh) - Automated release pipeline (validate-qa → promote-main)
