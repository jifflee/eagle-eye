...
Run `/issue:triage-bulk` later to classify, link to epics, and detect duplicates.
...
Pending triage - run /issue:triage-bulk to classify and organize.
```
...
Run `/issue:triage-bulk` to classify, set priority, and link to epics.
...
- **Data scripts:** `scripts/capture-data.sh`, `scripts/find-parent-issues.sh`, `scripts/search-similar-issues.sh`
- **API calls:** Batched duplicate detection and parent search
- **Savings:** ~40% reduction from inline gh searches
- **Performance tip:** Use `--bug`, `--feature`, or `--idea` flags to skip category detection
...
- **Deferred triage:** Issues created with `--fast` get `needs-triage` label for PM processing via `/issue:triage-bulk`
