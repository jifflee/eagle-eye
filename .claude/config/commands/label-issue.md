...
/label-issue <issue> --remove <label> --add <label>
/label-issue <issue> --validate          # Check label completeness
```
...
```bash
./scripts/label-issue-data.sh <issue_number>
```
...
- Single call to `./scripts/label-issue-data.sh` returns all label data
