...
# ─── Blocker Self-Reporting (Issue #1330) ─────────────────────────────────────
# When a container detects it is stuck (Claude timeout, watchdog kill, no output),
# it posts a structured comment to the GitHub issue and updates labels so an agent
# team or human can evaluate and triage the issue.

BLOCKER_REPORT_SCRIPT=""
for path in "${SCRIPT_DIR}/../container-blocker-report.sh" \
            "/workspace/repo/scripts/container-blocker-report.sh" \
            "/workspace/repo/.claude/scripts/container-blocker-report.sh"; do
    if [ -f "$path" ] && [ -x "$path" ]; then
        BLOCKER_REPORT_SCRIPT="$path"
        break
    fi
done

# Post a blocker report to the GitHub issue (stuck/hung container self-report)
# Arguments: phase reason [detail]
post_container_blocker() {
    local phase="${1:-implementation}"
    local reason="${2:-unknown}"
    local detail="${3:-}"

    if [ -z "$BLOCKER_REPORT_SCRIPT" ] || [ -z "${ISSUE:-}" ]; then
        log_warn "Blocker report skipped (script not found or ISSUE unset)"
        return 0
    fi
    ...
    STUCK_TIMEOUT="${STUCK_TIMEOUT:-${PHASE_TIMEOUT:-600}}" \
    HARD_TIMEOUT="${HARD_TIMEOUT:-${TOTAL_TIMEOUT:-1800}}" \
    "$BLOCKER_REPORT_SCRIPT" \
        --issue "$ISSUE" \
        --phase "$phase" \
        --reason "$reason" \
        ${detail:+--detail "$detail"} \
        2>/dev/null || log_warn "Blocker report failed (non-fatal)"
}
...
invoke_claude_with_retry() {
    ...
    while true; do
        ...
        if [ "$last_exit" -eq 124 ]; then
            ...
            # Self-report stuck state to GitHub issue for triage (Issue #1330)
            post_container_blocker "implementation" "claude_timeout" \
                "Claude did not respond within the allowed ${CLAUDE_TIMEOUT}s window"
...
Begin by using the Read tool on a relevant file, then use Write or Edit to make changes."
            ...
            if [ -n "$HEAD_BEFORE" ] && [ -n "$HEAD_AFTER" ] && [ "$HEAD_BEFORE" != "$HEAD_AFTER" ]; then
                ...
                if [ "$UNTRACKED_RETRY" -gt 0 ]; then
                    ...
                    # Self-report stuck state to GitHub issue for triage (Issue #1330)
                    post_container_blocker "implementation" "no_output" \
                        "Claude completed but produced no file changes after two attempts — issue may need clearer acceptance criteria"
            ...
            # Self-report stuck state to GitHub issue for triage (Issue #1330)
            post_container_blocker "implementation" "no_output" \
                "Claude completed but produced no file changes — issue may need clearer acceptance criteria"
...
# ============================================================================
# Phase 4.6: Post-Completion Documentation Review (Issue #1331)
# Trigger documentation agents as a background sub-process after PR creation.
# Non-fatal and non-blocking: runs asynchronously and does NOT delay the PR.
# Controlled by SKIP_DOCS=true environment variable or --skip-docs flag.
# ============================================================================
if [ "${SKIP_DOCS:-false}" != "true" ] && [ -n "${PR_NUMBER:-}" ] && [ -n "$ISSUE" ]; then
    log_phase "Phase 4.6: Post-Completion Documentation Review"
    watchdog_phase "doc-review"

    DOC_SCRIPT=""
    for doc_path in "${SCRIPT_DIR}/container-post-completion-docs.sh" \
                    "/workspace/repo/scripts/container/container-post-completion-docs.sh"; do
        if [ -f "$doc_path" ] && [ -x "$doc_path" ]; then
            DOC_SCRIPT="$doc_path"
            break
        fi
    done
    ...
    if [ -n "$DOC_SCRIPT" ]; then
        log_info "Post-completion doc review triggered for PR #${PR_NUMBER}"

        # Launch as background sub-process so it does NOT block PR creation or CI
        (
            "$DOC_SCRIPT" \
                --pr "$PR_NUMBER" \
                --issue "$ISSUE" \
        ...
        DOC_AGENT_PID=$!
        log_info "Documentation agent launched in background (PID: $DOC_AGENT_PID)"
        log_info "Doc review will push updates to PR #${PR_NUMBER} branch asynchronously"
    else
        log_warn "container-post-completion-docs.sh not found — skipping doc review"
    fi
else
    if [ "${SKIP_DOCS:-false}" = "true" ]; then
        log_info "Skipping post-completion doc review (SKIP_DOCS=true)"
    else
        log_info "Skipping post-completion doc review (no PR number available)"
    fi
fi
...
# ============================================================================
# Phase 7: Framework Auto-Update Check (Issue #1329)
# ============================================================================
# If a framework update was queued during sprint-work-preflight (every 25 runs),
# trigger it now — BEFORE returning so the orchestrator loop picks up the next
# issue with the latest framework version already installed.
# This runs on the HOST side; the script is in the same sprint/ directory.
_UPDATE_CHECK_SCRIPT="${SCRIPT_DIR}/../sprint/framework-update-check.sh"
if [ ! -x "$_UPDATE_CHECK_SCRIPT" ]; then
    # Fallback: same-dir layout (when called from container context)
    _UPDATE_CHECK_SCRIPT="${SCRIPT_DIR}/framework-update-check.sh"
fi
if [ -x "$_UPDATE_CHECK_SCRIPT" ] && [ "${SPRINT_UPDATE_CHECK_DISABLE:-}" != "true" ]; then
    set +e
    "$_UPDATE_CHECK_SCRIPT" trigger-if-queued 2>&1 || true
    set -e
fi
unset _UPDATE_CHECK_SCRIPT
